<nav id="navGlobal">
    <div class="fix">
        <ul class="cf">
            ...
            <li>
                <a class="{{(!empty($page) && ($page == $cate->id | in_array($page, $cate->subCategories->lists('id')))) ? 'active' : ''}}" href="{{url('chuyen-muc', $cate->slug)}}" title=""><span>{{ ($cate->slug == 'duoc-lieu-voi-benh-gan') ? 'Dược liệu' : $cate->name  }}</span></a>
